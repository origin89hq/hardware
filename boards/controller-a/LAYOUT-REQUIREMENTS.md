...
| 8 | **Inrush into `V3V3_ESP`** | **Open, from the rev A bench.** Rev A's `Q2` connects `C24` and the module to `V3V3` in nanoseconds. After 10 minutes off, every switch-on corrupted the MCU (22 of 22); switch-ons a few seconds or 30 s apart all passed. `A-23` needs a slew-controlled switch for rev B, about 1 ms or slower: a TPS22918 with `CT` = 1 nF, or `Q2` with a gate resistor and a gate-drain capacitor. Close when a rev A rework passes 10-minute off-times. Tracked in [origin89hq/hardware#5](https://github.com/origin89hq/hardware/issues/5); evidence in `bench/2026-09-14.md` |
