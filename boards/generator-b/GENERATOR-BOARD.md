...
| **B-09** | CN9's pin order **is Board A's**: 1 `+12V`, 2 `GND`, 3 `RUN`, 4 `KICK`, 5 `FEEDBACK`. **Verified against `Controller / Schematic1` for rev A**: there CN9 is 1 `V12`, 2 `GND`, 3 `GEN_RUN_CMD` (PD0), 4 `GEN_WDT_KICK` (PD1), 5 `GEN_STATUS` (PD2). Board A rev B keeps this order but moves the three logic nets off PD0–PD2 and protects them at its end (`A-34`), and pin 1 carries the bank voltage, labelled for that on both boards rather than `+12V` (`A-20b`). **This revision of board B is built for 12 V**: the `HT7533-1` and the `G5V-2-DC12` coils above are its documented power path, so a 24 V bank is allowed on this connector only once #33 defines board B rev B's input regulation, relay drive, TVS and ratings, and a rev A board B on a 24 V bank is a short through board A's limiter. Written on both silkscreens | Two boards that disagree on a locking connector's pin order is 12 V on a logic pin, with a connector that cannot be plugged the other way to fix it |
